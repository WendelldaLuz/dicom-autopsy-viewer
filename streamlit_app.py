...
def show_main_app():
    ...
    if 'user_data' not in st.session_state or st.session_state.user_data is None:
        st.error("❌ Dados de usuário não encontrados. Redirecionando para autenticação...")
        show_user_form()
        return
    
    user_data = st.session_state.user_data
    nivel_acesso = user_data.get('nivel_acesso', 'Usuário')
    nome_usuario = user_data.get('nome', 'Usuário Não Identificado')
    departamento = user_data.get('departamento', 'Departamento Não Especificado')
    ...
    # Inicializa a variável de estado para a navegação
    if 'page' not in st.session_state:
        st.session_state.page = "Análise de Imagem"
    
    # Barra lateral otimizada com navegação
    with st.sidebar:
        st.markdown(f"""
            <div class='user-card'>
                <div class='user-card-header'>
                    ...
                    <span class='user-badge'>{nivel_acesso}</span>
                </div>
                <div class='user-card-body'>
                    <p class='user-name'>{nome_usuario}</p>
                    <p class='user-dept'>{departamento}</p>
                    ...
                    <div class='user-permissions'>
                        ...
                        {('<span class="perm-badge">👑 Administração</span>' if nivel_acesso.lower() == 'admin' else '')}
                    </div>
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    # Conteúdo da página principal baseado na seleção da barra lateral
    if st.session_state.page == "Análise de Imagem":
        show_image_analysis_page(user_id)
    elif st.session_state.page == "Gerenciamento de Casos":
        show_case_management_page(user_id)
    elif st.session_state.page == "Configurações & Relatórios":
        show_reports_and_config_page(user_id)
    elif st.session_state.page == "Informações do Sistema":
        show_system_info_page()


def show_image_analysis_page(user_id):
    st.header("Análise de Imagem DICOM")
    st.markdown("<p>Carregue os arquivos para começar a análise.</p>", unsafe_allow_html=True)

    try:
        casos = load_user_cases(user_id)
        caso_options = [c['id'] for c in casos] if casos else []
        
        if not caso_options:
            st.warning("⚠️ Crie um caso na seção de 'Gerenciamento de Casos' para começar.")
            caso_selecionado = None
        else:
            caso_selecionado = st.selectbox(
                "1. Selecione um caso para análise:",
                options=caso_options,
                format_func=lambda x: f"Caso {x} - {next((c.get('status', 'N/A') for c in casos if c['id'] == x), 'N/A')}",
                help="Selecione um caso existente para associar os arquivos DICOM"
            )
            st.session_state.caso_selecionado = caso_selecionado
    except Exception as e:
        st.error(f"Erro ao carregar casos: {str(e)}")
        st.session_state.caso_selecionado = None
        return

    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    
    st.subheader("2. Upload de Exames DICOM")
    uploaded_files = st.file_uploader(
        "Selecione os arquivos DICOM para análise",
        type=['dcm', 'DCM'],
        accept_multiple_files=True,
        help=f"Máximo de {UPLOAD_LIMITS.get('max_files', 10)} arquivos DICOM ({UPLOAD_LIMITS.get('max_size_mb', 50)}MB cada)"
    )
    ...
    if uploaded_files:
        ...
        if not is_valid:
            ...
            if st.session_state.caso_selecionado:
                try:
                    # Registrar proveniência do upload
                    register_provenance_event(
                        case_id=st.session_state.caso_selecionado,
                        user_id=user_id,
                        event_type="upload",
                    ...
                    st.warning(f"Aviso: Não foi possível registrar evento de proveniência: {str(e)}")
    ...
    # Exibe a interface de análise apenas se houver arquivos e um caso selecionado
    if st.session_state.get('uploaded_files') and st.session_state.get('caso_selecionado'):
        selected_file = st.selectbox(
            "Selecione o exame para análise detalhada:",
            [f.name for f in st.session_state.uploaded_files],
            key="file_selector"
        )
        dicom_file = next((f for f in st.session_state.uploaded_files if f.name == selected_file), None)
        ...
        if dicom_file:
            try:
                ...
                try:
                    ...
                    # Sistema de abas para análise
                    tab1, tab2 = st.tabs(["👁️ Visualização", "⚙️ Metadados"])
                    with tab1:
                        st.header("Visualização Médica Avançada")
                        if hasattr(dataset, 'pixel_array'):
                            image = dataset.pixel_array
                            create_medical_visualization(image, f"Exame: {selected_file}")
                        else:
                            st.warning("⚠️ Arquivo DICOM não contém dados de imagem visível.")
                    
                    with tab2:
                        st.header("Metadados Técnicos do Exame")
                        tech_info = { 'Modalidade': safe_dicom_value(dataset, 'Modality') }
...
def show_case_management_page(user_id):
    st.header("Gerenciamento de Casos")
    st.markdown("<p>Crie, visualize e gerencie todos os casos forenses.</p>", unsafe_allow_html=True)

    try:
        casos = load_user_cases(user_id)
        if st.button("➕ Novo Caso", use_container_width=True, disabled=not user_id):
            novo_caso_id = create_new_case(user_id)
            st.success(f"Caso {novo_caso_id} criado com sucesso!")
            st.rerun()

        st.subheader("Casos Existentes")
        if not casos:
            st.info("Nenhum caso encontrado. Crie um novo caso para começar.")
        else:
            for caso in casos:
                st.markdown(f"**Caso {caso['id']}** - Status: *{caso['status']}*")
                # Botões ou informações adicionais para cada caso aqui
    except Exception as e:
        st.error(f"Erro ao carregar ou gerenciar casos: {str(e)}")

def show_reports_and_config_page(user_id):
    st.header("Configurações e Relatórios")
    st.markdown("<p>Adicione personalizações e gere relatórios de análise.</p>", unsafe_allow_html=True)
    
    st.subheader("Personalização de Relatório")
    ...
    if uploaded_logo:
        try:
            ...
            st.error(f"Erro ao processar logotipo: {str(e)}")
            
    st.subheader("Exportar Relatório")
    if st.session_state.get('caso_selecionado') and st.session_state.get('uploaded_files'):
        # Lógica para exportar relatórios (botões de download/e-mail)
        st.button("📥 Baixar Relatório PDF", use_container_width=True)
        st.button("📧 Enviar por Email", use_container_width=True)
    else:
        st.warning("⚠️ Carregue arquivos e selecione um caso para gerar relatórios.")

def show_system_info_page():
    st.header("Informações do Sistema")
    st.markdown("<p>Visualize o status do sistema e os eventos de proveniência.</p>", unsafe_allow_html=True)
    st.info("Aqui seriam exibidas as informações do sistema, logs de auditoria e proveniência.")
    
# ---
# O resto do código (main e __name__ == "__main__") permanece inalterado.
# ---
